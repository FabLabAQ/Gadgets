...
const unsigned long PROGMEM display_string[STRING_LENGTH] = {char_F, char_A, char_B, char_L, char_A, char_B, char_A, char_Q};
...
void renderString(int offset) {
  int index = 0;
  while (display_string[index]) {
    renderCharacter(display_string[index], offset - index * (DISPLAY_WIDTH + 1));
    index++;
  }
}
